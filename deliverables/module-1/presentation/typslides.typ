...
#let typslides(
  ...
  // change citation number size
  show cite: it => (
    context {
      text(size: 0.7em,baseline: -.1em,)[ #it ]
    }
  )

  show footnote: it => (
    context {
      text(fill: theme-color.get(), size: 1.2em)[#it]
    }
  )
